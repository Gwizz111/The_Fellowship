...
app.get("/printFavorites", async (_req, res) => {
  interface Favorites {
    quoteId: string;
    quoteDialog: string;
    characterName: string;
  ...
  .db("fellowship")
  .collection("favorites")
  .find({userId: new ObjectId(_req.session.userId)})
  .toArray();
  let quotesArray = favorites[0].quoteId;
  let quotesDialog: string[] = [];
  let characterIds : string[] = [];
  ...
  res.type("text/html");
  res.render("../public/views/printFavorites.ejs", {quoteDialogAndCharacter});
});

app.get("/printBlacklist", async (req, res) => {
  let user = await client
  .db("fellowship")
  .collection("users")
  .findOne({_id: new ObjectId(req.session.userId)});
  ...
  console.log(quoteDialogAndCharacter)
  res.type("text/html");
  res.render("../public/views/printBlacklist.ejs", {quoteDialogAndCharacter});
});
